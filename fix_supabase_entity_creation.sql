-- Fix Supabase Entity Creation Issues
...
-- Step 4: Create helper functions for RLS to avoid recursion
...
CREATE POLICY "Users can update company applicants" ON applicants
    FOR UPDATE USING (company_id = auth.user_company_id());

CREATE POLICY "Users can delete company applicants" ON applicants
    FOR DELETE USING (company_id = auth.user_company_id());
